"""Email-safe copy of the Sprag daily brief.
...
Writes daily-09-01-2026.email.html and daily-09-01-2026.email.txt.
"""
import html
import re
import pathlib
...
SRC = HERE / "daily-09-01-2026.html"
OUT_HTML = HERE / "daily-09-01-2026.email.html"
OUT_TXT = HERE / "daily-09-01-2026.email.txt"
REPORT_URL = "https://415brand.com/analysis/sprag/daily-09-01-2026"
...
def build():
    ...
    out = []
    out.append(p("Deepgram daily brief", f"font-family:{FONT};font-size:26px;line-height:1.2;font-weight:bold;color:#202122", "0 0 4px"))
    out.append(p("For Ian and Alex at Sprag, September 1, 2026", MUTED + ";padding-bottom:10px;border-bottom:1px solid #c8ccd1", "0 0 18px"))
    ...
    pattern = re.compile(
        ...
        r'|(?P<changes><ul class="changes">(?P<changest>.*?)</ul>)',
        re.S,
    )
    ...
    def emit(m, depth=0):
        ...
        if g["h2"]:
            ...
            for li in re.findall(r'<li[^>]*>(.*?)</li>', g["changest"], re.S):
                out.append(p('&bull;&nbsp; ' + inline_inner(li), BODY, '0 0 8px'))
        elif g["links"]:
    ...
    out.append(p(
        f'Full report online: <a href="{REPORT_URL}" style="{LINK}">{REPORT_URL}</a><br><br>'
    ...
    doc = (
        ...
        "<title>Deepgram daily brief for Sprag, September 1, 2026</title></head>"
    ...
    # the green rule in a why block is a 3px spacer cell, not a column
    txt = re.sub(r'<td width="3"[^>]*>&nbsp;</td>', "", body_html)
    txt = re.sub(r"<br\s*/?>", "\n", txt)
